...
/**
  ...
  * @version $Id: Decreasing.java,v 1.1.1.1 2002/06/28 07:36:16 waleed Exp $
  */
...
public class Decreasing implements PepI {

    static final String name = "dec"; 
    static final String description = "Finds period of decrease."; 
     ...
     * This decludes the description of the number of parameters, 
     * the names of the parameters and other info. Note also that
     * this is decluded free with any EventVec's we return. 
     * 
     */
        ...
        while(currentPoint < difference.length){ 
            ...
            if(misses > noiseIgnore){
                ...
                if(endPoint-startPoint +1 > minDurn){
                    ev.add(makeDec(startPoint, endPoint-1, c, difference, useRelativeTime, offset)); 
                }
                currentPoint++; 
    ...
    EventI makeDec(int start, int end, ChannelI c, float[] diff, boolean useRelativeTime, float offset){
        ...
        return new DecEvent(yave-offset, midTime, gradient, duration); 
    } 
}

class DecEvent implements EventI {
    float avg; 
    float midTime; 
    float gradient; 
    float duration; 
    
    DecEvent(float avg, float midTime, float gradient, float duration){
        this.avg = avg; 
    ...
    public String toString(){
        return "Decreasing: midTime = " + midTime +  " avg = " + avg + 
