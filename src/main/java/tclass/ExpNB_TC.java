 ...
 * @version $Id: ExpNB_TC.java,v 1.1.1.1 2002/06/28 07:36:16 waleed Exp $
 */
...
public class ExpNB_TC  {
    ...
    public static void main(String[] args) throws Exception {
        ...
        NaiveBayes[] nbLearners = new NaiveBayes[numClasses]; 
        for(int i=0; i < numClasses; i++){
            nbLearners[i] = new NaiveBayes(); 
            nbLearners[i].setDomDesc(domDesc);
            nbLearners[i].setParam("numDivs",thisExp.numDivs); 
            nbLearners[i].setAttDescVec(trainAttsByClass[i].getStreamAttValVec().getDescription()); 
        }

        // Now learn. 

        ClassifierI[] nbClassifiers = new ClassifierI[numClasses]; 
        for(int i=0; i < numClasses; i++){
            nbClassifiers[i] =
                nbLearners[i].learn(trainAttsByClass[i]); 
